...
def run_simulation(dithering, source, source_alt, source_az, boresight_alt, boresight_az):
    check_angles_primary   = {0: [-30., -150.], 1: [-30., 90], 2: [-150., 90.], 3: [-30., -150.]}
    check_angles_secondary = {0: [0, 90, 180], 1: [-90, 150, 30], 2: [-90, 150, 30], 3: [30, 150, -90]}
    ...
    new_angles_secondary = check_angles_secondary[max_idx]
    for i in range(2):
        x_dither = -1*search_radius*np.cos(new_angles_primary[i]*u.deg)
        y_dither = search_radius*np.sin(new_angles_primary[i]*u.deg)
        dithering.place_fiber([x_offset+x_dither, y_offset+y_dither])
        dithering.run_simulation(source_type, *source, report=False)
        SNR.append(np.median(dithering.SNR['b'][0]))
        x.append(x_dither)
        y.append(y_dither)
    for i in range(3):
        x_dither = -1*search_radius/2.*np.cos(new_angles_secondary[i]*u.deg)
        y_dither = search_radius/2.*np.sin(new_angles_secondary[i]*u.deg)
        dithering.place_fiber([x_offset+x_dither, y_offset+y_dither])
        dithering.run_simulation(source_type, *source, report=False)
        SNR.append(np.median(dithering.SNR['b'][0]))
        x.append(x_dither)
